...
{ 
    ...
    { try {
        ...
        String message = "npcinfo";
        message += "," + npc.getX();
        message += "," + npc.getY();
        message += "," + npc.getZ();
        message += "," + npc.getSize(); // float like 0.5 or 2.0
        sendPacketToAll(message);
    } catch (IOException e) {
        System.out.println("Failed to send npcinfo message");
        e.printStackTrace();
    }
 }
    ...
    { try {
        String[] pos = {
            String.valueOf(npcCtrl.getNPC().getX()),
            String.valueOf(npcCtrl.getNPC().getY()),
            String.valueOf(npcCtrl.getNPC().getZ())
        };
        System.out.println("Sending createNPC to client " + clientID);
        String message = "createNPC";
        message += "," + pos[0];
        message += "," + pos[1];
        message += "," + pos[2];
        sendPacket(message, clientID); // This targets only the requesting client
    } catch (IOException e) {
        System.out.println("Failed to send NPC start message");
        e.printStackTrace();
    } }
